...
BOOL CloseTo(double a, double b) { return fabs(a - b) < 20; }

const SimpleWindowGeometryChangeBlock shiftItLeft =
    ^AnchoredRect(NSRect windowRect, NSSize screenSize) {
      NSRect r = NSMakeRect(0, 0, 0, 0);

      r.origin.x = 0;
      r.origin.y = 0;

      double windowWidth = windowRect.size.width;
      double screenWidth = screenSize.width;

      r.size.width = screenSize.width / 2;
      r.size.height = screenSize.height;

      if (CloseTo(windowRect.origin.x, r.origin.x) &&
          CloseTo(windowRect.size.height, r.size.height)) {
        if (CloseTo(windowWidth, screenWidth / 2)) {
          r.size.width = floor(screenWidth / 3);
        } else if (CloseTo(windowWidth, screenWidth / 3)) {
          r.size.width = floor(screenWidth / 4);
        }
      }
...
const SimpleWindowGeometryChangeBlock shiftItRight =
    ^AnchoredRect(NSRect windowRect, NSSize screenSize) {
      NSRect r = NSMakeRect(0, 0, 0, 0);
      ...
      if (CloseTo(windowRect.origin.x + windowRect.size.width,
                  r.origin.x + r.size.width) &&
          CloseTo(windowRect.size.height, r.size.height)) {
        if (CloseTo(windowWidth, screenWidth / 2)) {
          r.size.width = floor(screenWidth / 3);
          r.origin.x = screenWidth - (screenWidth / 3);
        } else if (CloseTo(windowWidth, screenWidth / 3)) {
          r.size.width = floor(screenWidth / 4);
          r.origin.x = screenWidth - (screenWidth / 4);
        }
      }
...
const SimpleWindowGeometryChangeBlock shiftItBottom =
    ^AnchoredRect(NSRect windowRect, NSSize screenSize) {
      NSRect r = NSMakeRect(0, 0, 0, 0);
...
const SimpleWindowGeometryChangeBlock shiftItTopLeft =
    ^AnchoredRect(NSRect windowRect, NSSize screenSize) {
      NSRect r = NSMakeRect(0, 0, 0, 0);
...
const SimpleWindowGeometryChangeBlock shiftItTopRight =
    ^AnchoredRect(NSRect windowRect, NSSize screenSize) {
      NSRect r = NSMakeRect(0, 0, 0, 0);
...
const SimpleWindowGeometryChangeBlock shiftItBottomLeft =
    ^AnchoredRect(NSRect windowRect, NSSize screenSize) {
      NSRect r = NSMakeRect(0, 0, 0, 0);
...
const SimpleWindowGeometryChangeBlock shiftItBottomRight =
    ^AnchoredRect(NSRect windowRect, NSSize screenSize) {
      NSRect r = NSMakeRect(0, 0, 0, 0);
...
const SimpleWindowGeometryChangeBlock shiftItFullScreen =
    ^AnchoredRect(NSRect windowRect, NSSize screenSize) {
      NSRect r = NSMakeRect(0, 0, 0, 0);
...
const SimpleWindowGeometryChangeBlock shiftItCenter =
    ^AnchoredRect(NSRect windowRect, NSSize screenSize) {
      NSRect r = NSMakeRect(0, 0, 0, 0);
...
- (AnchoredRect)shiftWindowRect:(NSRect)windowRect
                     screenSize:(NSSize)screenSize
                    withContext:(id<SIWindowContext>)windowContext {
  double kw = 0;
  double kh = 0;
  ...
  if (kw <= 0) {
    FMTLogError(@"Invalid size for width delta: %f (type: %ld)", kw,
                sizeDeltaType);
    return MakeAnchoredRect(windowRect, 0);
  }

  if (kh <= 0) {
    FMTLogError(@"Invalid size for height delta: %f (type: %ld)", kh,
                sizeDeltaType);
    return MakeAnchoredRect(windowRect, 0);
  }
  ...
  int directions =
      kLeftDirection | kTopDirection | kBottomDirection | kRightDirection;
  ...
  if (directions & kLeftDirection && directions & kRightDirection) {
    khorz /= 2;
  }
  ...
  if (directions & kTopDirection && directions & kBottomDirection) {
    kvert /= 2;
  }
  ...
  r.size.width =
      r.size.width > screenSize.width ? screenSize.width : r.size.width;

  r.size.height = (CGFloat)(r.size.height < kh ? kh : r.size.height);
  r.size.height =
      r.size.height > screenSize.height ? screenSize.height : r.size.height;

  r.origin.x = r.origin.x < 0 ? 0 : r.origin.x;
  r.origin.x = r.origin.x > screenSize.width - r.size.width
                   ? screenSize.width - r.size.width
                   : r.origin.x;

  r.origin.y = r.origin.y < 0 ? 0 : r.origin.y;
  r.origin.y = r.origin.y > screenSize.height - r.size.height
                   ? screenSize.height - r.size.height
                   : r.origin.y;
...
- (BOOL)execute:(id<SIWindowContext>)windowContext error:(NSError **)error {
  ...
  if (![windowContext getFocusedWindow:&window error:&cause]) {
    *error = SICreateErrorWithCause(kShiftItActionFailureErrorCode, cause,
  ...
  if (!flag) {
    *error =
        SICreateError(kShiftItActionFailureErrorCode, @"Window cannot zoom");
    return NO;
  }
...
- (BOOL)execute:(id<SIWindowContext>)windowContext error:(NSError **)error {
  ...
  if (![window canEnterFullScreen:&flag error:&cause]) {
    *error = SICreateErrorWithCause(
        kShiftItActionFailureErrorCode, cause,
        @"Unable to find out if window can enter fullscreen");
  }
  if (!flag) {
    *error = SICreateError(kShiftItActionFailureErrorCode,
                           @"Window cannot enter fullscreen");
    return NO;
  }
...
- (BOOL)execute:(id<SIWindowContext>)windowContext error:(NSError **)error {
  ...
  if ([window respondsToSelector:@selector(getFullScreen:error:)]) {
    ...
    if (flag) {
      *error = SICreateError(kShiftItActionFailureErrorCode,
                             @"Windows in fullscreen are not supported");
      return NO;
    }
  }
  ...
  if (![window getGeometry:&currentGeometry
  ...
  FMTLogInfo(@"Current window geometry: %@ screen: %@",
             RECT_STR(currentGeometry), currentScreen);

  NSSize currentScreenSize = [currentScreen size];
  SIScreen *screen =
      next_ ? [currentScreen nextScreen] : [currentScreen previousScreen];
  ...
  if (![window setGeometry:geometry screen:screen error:&cause]) {
    *error = SICreateErrorWithCause(
        kShiftItActionFailureErrorCode, cause,
        @"Unable to set new geometry of a window to %@ at screen: %@",
        RECT_STR(geometry), screen);
    return NO;
  }
